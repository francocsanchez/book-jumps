import { listConceptosEgresos, listConceptosIngresos } from "@/api/ConceptosAPI";
...
import { generarMovimientoIngreso } from "@/api/MovimientoAPI";

export default function GenerarIngresoModal() {
  const queryClient = useQueryClient();
  const navigate = useNavigate();
  const location = useLocation();
  const queryParams = new URLSearchParams(location.search);
  const viewModal = queryParams.get("generarIngreso");
  const showModal = viewModal ? true : false;

  const { data: dataConceptos = [], isLoading: loadConceptos } = useQuery({
    queryKey: ["conceptos-ingresos"],
    queryFn: listConceptosIngresos,
  });
  ...
  const mutation = useMutation({
    mutationFn: generarMovimientoIngreso,
    onError: (err) => {
